      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 w-[90%] max-w-[360px] h-[450px] rounded-2xl border border-white/10 glass-card bg-slate-950/90 overflow-hidden shadow-2xl z-50 flex flex-col font-sans"
          >
            {/* Window Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-slate-950/60 select-none">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-gradient-to-tr from-primary to-secondary flex items-center justify-center border border-white/10">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3.5 scrollbar-thin">
                ...
                >
                  ...
                  <div
                    className={`p-3 rounded-2xl text-xs leading-relaxed ${
                      msg.sender === "user"
                        ? "bg-primary/20 border border-primary/20 text-slate-200 rounded-tr-none"
              ...
              {isTyping && (
                <div className="flex gap-2.5 max-w-[85%]">
                  <div className="w-7 h-7 rounded-full bg-cyan-950/40 border border-cyan-500/20 flex items-center justify-center text-cyan-300">
                    <Bot className="w-3.5 h-3.5" />
                  </div>
                  <div className="bg-slate-900 border border-white/5 p-3 rounded-2xl rounded-tl-none flex items-center gap-1.5 h-8">
            ...
            {messages.length === 1 && (
              <div className="px-4 py-2 flex flex-col gap-1.5 bg-slate-950/40 border-t border-white/5 select-none">
                {suggestions.map((sug, idx) => (
                  <button
                    key={idx}
                    onClick={() => handleSendMessage(sug)}
                    className="text-left text-[10px] text-cyan-300 bg-cyan-500/5 hover:bg-cyan-500/10 border border-cyan-500/10 hover:border-cyan-500/20 px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer"
                  >
                    {sug}
                  </button>
                ))}
            ...
            <form
              ...
              className="p-3 border-t border-white/10 bg-slate-950/50 flex items-center gap-2"
            >
              <input
                type="text"
                className="flex-1 bg-transparent border-none outline-none text-xs text-slate-200 placeholder-slate-600 font-sans"
              ...
              <button
                type="submit"
                className="p-1.5 rounded-lg bg-cyan-500/10 border border-cyan-500/20 hover:bg-cyan-500/20 text-cyan-400 hover:text-cyan-200 transition-colors cursor-pointer"
