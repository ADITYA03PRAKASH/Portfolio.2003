...
function StatCounter({ target, label, suffix }: StatItem) {
  ...
  return (
    <div ref={ref} className="flex flex-col items-center p-6 rounded-2xl glass-card border border-white/5 relative group overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
      <span className="font-heading text-4xl sm:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary drop-shadow-[0_0_15px_rgba(6,182,212,0.15)] select-none">
        {count}
        {suffix}
      </span>
    ...
    <section id="about" className="relative py-24 px-6 md:px-12 w-full max-w-7xl mx-auto overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-10 items-start">
        ...
        <div className="md:col-span-6 lg:col-span-3 flex justify-center">
          <div className="w-full max-w-[260px] rounded-3xl border border-white/5 glass-card p-2 relative overflow-hidden group shadow-2xl">
            ...
            <div className="absolute bottom-4 left-4 right-4 bg-slate-950/70 backdrop-blur-md border border-white/10 rounded-xl px-3 py-2 text-center text-[10px] uppercase font-bold tracking-widest text-cyan-200">
              Aditya Dwivedi
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-12 lg:col-span-4 flex flex-col gap-8 mt-8 lg:mt-0">
          ...
          <div className="relative border-l border-white/10 pl-6 space-y-8 py-1">
            {journey.map((item, idx) => (
              ...
              >
                ...
                <div className="absolute -left-[35px] top-1.5 w-[18px] h-[18px] rounded-full bg-slate-950 border-2 border-white/20 flex items-center justify-center group-hover:border-cyan-400 transition-colors z-10 shadow-[0_0_10px_rgba(0,0,0,0.8)]">
                  <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
                
                {/* Milestone Details Card */}
                <div className="p-5 rounded-2xl glass-card border border-white/5 hover:border-white/10 transition-colors">
                  <div className="flex items-center justify-between gap-4 mb-2 flex-wrap sm:flex-nowrap">
                    <span className="text-[10px] uppercase font-bold tracking-wider text-cyan-400 bg-cyan-500/5 px-2 py-0.5 rounded border border-cyan-500/10">
                      {item.year}
                    </span>
                  ...
                  <h4 className="font-heading text-sm sm:text-base font-bold text-slate-200">
                    {item.title}
                  </h4>
